...
|---|---|---|---|---|---|
| **O2** | Onboarding system — five-question problem statement, org-wide project tree, background Fable advisor (harness + tool import recommendations), HITL-gated imports on top of R17 versions, optional hypotheses | Planned (design settled 2026-09-06) | [O2-project-tree-advisor.md](docs/plans/O2-project-tree-advisor.md) | `feat/O2-project-tree` | — |
| **R19** | Guided evolution search — proactive, goal-conditioned evolution proposals over the R17 version tree (HyperAgents-style archive + eval folder + meta-planner), decided by the human or by an independent LLM judge on another provider; sequential goals from the onboarding brief | 🟢 Backend + ui3 Plan panel built; offline-verified (2026-09-07: 136 tests green, incl. a regression suite for an adversarial review pass). Pending: platform-scope staging/cutover (Phase 4, flag off) and a live judge run (the supplied OpenAI key is rejected with 401) | [R19-guided-evolution-search.md](docs/plans/R19-guided-evolution-search.md) · [reference note](docs/research/self-improvement-archives.md) | `feat/R19-guided-evolution` | `../coscientist-R19` |
