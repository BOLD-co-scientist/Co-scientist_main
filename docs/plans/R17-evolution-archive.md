...
- [x] 7. **Session continuity across switch**: `schema_version` stamp on new sessions; too-new sessions open read-only (runtime guard + a `readonly` flag on `/sessions` + a **display-only** UI banner). Fork endpoint (`POST /sessions/{sid}/fork`) exists and is verified, but the **recovery UX is deliberately deferred** (per user, 2026-08-31): the read-only case is currently untriggerable (nothing bumps `SCHEMA_VERSION`; the compat gate blocks accidental breaks), so we surface read-only only and design the recovery (fork, or otherwise) from real experience once it actually happens. The backend guard — the load-bearing safety invariant — stays.
