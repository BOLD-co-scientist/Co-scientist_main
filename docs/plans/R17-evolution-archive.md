...
- **Switch vs the long-lived API server.** Per-turn subprocesses pick up a switch automatically; the API-server process (Tier-2 code) does not until restarted. Decide: switch triggers an API reload, or a switch is *defined* to affect only subprocess-spawned work. (Impl-time.)
