...
### Tier 2 — Evolvable harness (capabilities — the point of the system)
Version-bound, grown by evolution, per-user repo, **swapped by a version switch**: MCP tools, subagent roles, workflows/methodology, prompts, critic/synthesis layers, models/tiers, `scaffold/` logic, `research/`+`evolution/` runtimes, **`api/` (HTTP+SSE), `ui3/` (the whole UI)**. The UI and API do **not** need cross-version freezing — version B ships B's API *and* B's UI together.
...
**CAN (Tier 2):** add a `critic` subagent that reviews each researcher report; add a plotting/literature-search MCP tool; change the checkpoint methodology; add a synthesis phase; swap the memory retrieval algorithm (BM25 → hybrid) keeping the JSONL schema; restyle the whole UI; rewrite the supervisor prompt.
...
- [ ] 2. **Fix skills as Tier-2 harness:** make skill authoring commit the SKILL.md into the active version (lightweight, HITL-gated evolution); track + seed base skills at tenant bootstrap; add a test that a committed skill travels with its version across a switch, and that `state/` (Tier-3) is byte-identical across a checkout of two versions. (Correctness prerequisite.)
...
- **Switch vs long-lived processes — OPEN (dedicated discussion later, once we understand real cases).**
  - *Settled (minimal) part:* the frozen harness knows **nothing** about an "agent/platform layer" split — that taxonomy stays out of the contract. **Reload-need is computed, not stored**: at switch time, `git diff <current> <target>`; if it touches a file a long-lived process (the API server) imports → a reload is needed, else not. Pure function of the two git trees, no meta field. Agent-layer changes (roles/prompts/skills/tools/methodology) never hit that set → self-apply on the next per-turn subprocess with zero reload.
  - *OPEN part:* **how** to reload when a long-lived process is expensive/disruptive to restart. Example: the API server holding many live SSE streams (an in-place restart drops every watcher's connection); worse if a future process holds a warm index / connection pool / an in-flight long computation. Options to weigh against concrete cost profiles: (a) in-place graceful restart — cheap **iff** the process is ~stateless; (b) **blue-green via a temporary worktree** — boot the target version's process from a parallel checkout, drain + hand off (SSE reconnect, finish in-flight), retire the old → zero-downtime, and the worktree gives the isolated boot environment for free; (c) run multiple versions' processes side-by-side and route. Defer the choice until we know what the long-lived processes actually hold and how costly a restart is. v1 may simply notify ("restart to fully apply") rather than automate any of this.
- **~~Skill visibility across the DAG~~ — DECIDED:** skills are Tier-2 harness (version-bound, like prompts); visible on the authoring node + its descendants. Authoring commits into the version.
