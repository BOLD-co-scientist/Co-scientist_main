...
- [~] 9. **Backend verification.** `docker compose up -d coscientist-api`; POST a research session whose task asks for a short report; tail `events.jsonl`; assert `results/` ends up with both a `.tex` and a `.pdf`. Then re-run with `COSCIENTIST_OUTPUT_FORMAT=markdown` and assert a `.md` (no compile). Record commands + outcome under **Notes**.
...
- **Step 9 remaining (`[~]`).** The full *live-agent* run (agent emits `.tex` + calls `latex_compile` → real `.pdf`) needs the image rebuilt with texlive AND API spend. Building `yuhe_coscientist:r13-verify` to prove texlive installs in a non-root image; the live one-shot against a texlive-equipped API needs the user's actively-running stack rebuilt/restarted, so it's left for the human to trigger (or approve) rather than disrupting their session unprompted.
