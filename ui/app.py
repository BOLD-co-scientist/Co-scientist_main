...
with st.sidebar:
    ...
    # 2. MIDDLE: Session History
    st.header("Session History")
    if STATE_DIR.exists():
        sessions = sorted([d.name for d in STATE_DIR.iterdir() if d.is_dir()], reverse=True)
        if not sessions:
...
if st.session_state.session_id is None:
        ...
        try:
            with open(events_file, "r") as f:
                ...
                if lines:
                    ...
                    if last_event.get("kind") in ["session.ended", "session.end", "session.error", "session.fatal"]:
                        is_dead = True
        except Exception:
            pass
    ...
    # Input for mid-session directives
