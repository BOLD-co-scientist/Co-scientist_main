...
- `fs_write_workspace.write` artifacts (CSVs, text/markdown summaries, PNG plots saved with matplotlib's `Agg` backend) into the session's `results/` dir. Match whatever output format the supervisor specifies for the final deliverable; don't hand back a full LaTeX document unless asked — your job is the numbers and the artifacts, the supervisor assembles the report.
